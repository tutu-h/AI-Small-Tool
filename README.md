...
- 网页端支持浏览器 DOM 读取优先：不抓接口，只读取页面已显示文本，失败再回退 OCR。
- 支持导入截图识别，方便排查或离线分析。
...
如果使用网页端，推荐点击工具里的 `启动Boss专用浏览器`。它会打开一个带远程调试端口的 Edge/Chrome 专用窗口，你在里面正常登录 Boss 直聘并停留在消息页。之后工具会优先读取这个浏览器里的 DOM 文本，不需要每次依赖截图 OCR。
...
- `启动Boss专用浏览器`：打开一个专用于 DOM 读取的 Boss 网页窗口，推荐网页端优先使用。
...
- 网页端会先尝试 DOM 读取：连接专用浏览器，读取页面已经渲染出来的候选人列表和聊天文本。
...
- 网页端不想频繁切前台：点击 `启动Boss专用浏览器`，在打开的专用窗口登录 Boss，工具会优先走 DOM 读取。
- DOM 读取为空：确认专用浏览器窗口还开着，Boss 页面停留在消息页，并且没有被验证码/登录页挡住。
